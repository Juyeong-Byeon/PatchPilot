...
1. Build a runner image that includes the real agent CLI. For the Codex-backed
   ...
   staged gstack pipeline (plan → implement → review → verify, one Codex pass per
   gstack skill) set `GSTACK_ARGS=/opt/runner/apps/runner/dist/gstack-staged-runner.js`
   instead of the single-pass `codex-agent-runner.js`.
