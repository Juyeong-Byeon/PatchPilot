            ...
            {spans.map((span) => {
              ...
              >
                ...
                <div className="relative h-8 overflow-hidden rounded-full bg-linen">
                  <div
                    className={`absolute top-1 bottom-1 flex min-w-[34px] items-center rounded-full px-3 ${spanClassName(span.status)}`}
                    style={spanStyle(span)}
                  >
                    <span className="truncate">{span.latestMessage || copy.spanNoEvents}</span>
                  </div>
                </div>
